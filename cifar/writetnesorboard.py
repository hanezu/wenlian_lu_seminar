...
class LogTensorboard(extension.Extension):
    ...
    def __init__(self, keys=None, trigger=(1, 'epoch'), postprocess=None,
                 distribution_trigger=10000, log_name='log', logger=None):
        self._keys = keys
        self._trigger = trigger_module.get_trigger(trigger)
        self._postprocess = postprocess
        self._distribution_trigger = distribution_trigger
    ...
    def __call__(self, trainer):
        ...
        for k, v in observation.items():
            try:
                ...
                self._logger.add_scalar(k, chainer.cuda.to_cpu(observation[k]), trainer.updater.iteration)
        if trainer.updater.iteration % self._distribution_trigger == 0:
            for name, param in model.namedparams():
                self._logger.add_histogram(name, chainer.cuda.to_cpu(param.data), trainer.updater.iteration, 'auto')
