...
import resnet50
import cv2
# import cifar.models.VGG as VGG
...
class PreprocessedDataset(chainer.dataset.DatasetMixin):
    ...
    def original_get_example(self, i):
    ...
    def get_example(self, i):
        ...
        ch, h, w = image.shape
        if ch == 1:
            image = cv2.cvtColor(image.transpose((1, 2, 0)), cv2.COLOR_GRAY2BGR).transpose((2, 0, 1))
        image_resized = cv2.resize(image.transpose((1, 2, 0)), (256, 256)).transpose((2, 0, 1))
        image_resized -= self.mean  # mean.shape = (3, 256, 256)d
        image_resized *= (1.0 / 255.0)  # Scale to [0, 1]
